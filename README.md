...
| `/[remove-]lgtm`                  | `/lgtm`</br>`/remove-lgtm`                         | Applies or removes the 'lgtm' labels to an PR or issue. For PRs, the 'lgtm' label is automatically removed when new commits are pushed. When a PR has both 'lgtm' and 'approved' labels, it will be automatically merged. **Labels need to be created manually in advance.** | label-lgtm             |
